...
import cn.zero.shadowkcp.ToyVpnUdpService;
import cn.zero.shadowkcp.dns.DnsPacket;
import cn.zero.shadowkcp.dns.Question;
import cn.zero.shadowkcp.dns.Resource;
import cn.zero.shadowkcp.dns.ResourcePointer;
...
//                    LocalVpnService.Instance.writeLog("Parse dns error: %s", e);
    ...
    private int getOrCreateFakeIP(String domainString) {
        ...
        if (fakeIP == null) {
            ...
            do {
                fakeIP = CommonMethods.ipStringToInt("10.231.0.0") | (hashIP & 0x0000FFFF);
                hashIP++;
...
//                        System.out.printf("FakeDns: %s=>%s(%s)\n", question.Domain, CommonMethods.ipIntToString(realIP), CommonMethods.ipIntToString(fakeIP));
                    Log.e("HongLi","\"FakeDns: "+question.Domain+"=>"+CommonMethods.ipIntToString(realIP)+"("+CommonMethods.ipIntToString(fakeIP)+")");
                    return true;
                }
            }
        }
...
//            LocalVpnService.Instance.sendUDPPacket(ipHeader, udpHeader);
            ToyVpnUdpService.getInstance().sendUDPPacket(ipHeader, udpHeader);
        }
    }
...
//                    System.out.printf("interceptDns FakeDns: %s=>%s\n", question.Domain, CommonMethods.ipIntToString(fakeIP));
                Log.e("HongLi","interceptDns FakeDns: "+question.Domain+"=>"+CommonMethods.ipIntToString(fakeIP)+"\n");
...
//                LocalVpnService.Instance.sendUDPPacket(ipHeader, udpHeader);
    ...
    public void onDnsRequestReceived(IPHeader ipHeader, UDPHeader udpHeader, DnsPacket dnsPacket) {
        if (!interceptDns(ipHeader, udpHeader, dnsPacket)) {
            ...
            Log.e("HongLi","remoteAddress:" + remoteAddress.getHostName() + ";port:" + remoteAddress.getPort());
...
//                if (LocalVpnService.Instance.protect(m_Client)) {
                if (ToyVpnUdpService.getInstance().protect(m_Client)) {
