 ...
 */
#include "cn_zero_shadowkcp_NativeHelper.h"
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <unistd.h>
...
//-----------------------------------------------------------------------------
//JNIEXPORT jstring JNICALL Java_cn_zero_shadowkcp_NativeHelper_getCLanguageString(JNIEnv *env, jobject obj){
//    return *env.NewStringUTF(env,"This just a test for Android Studio NDK JNI developer!");
//}

JNIEXPORT jstring JNICALL Java_cn_zero_shadowkcp_NativeHelper_startServer
        (JNIEnv *, jobject){
    char* param[6] = {"123","1400","10.0.3.0:32","0.0.0.0:0","8.8.8.8","vpnserver"};
    startServer(5,param);
}
...
int startServer(int argc, char* argv[])
{
    if (argc < 5) {
        printf("Usage: %s <tunN> <port> <secret> options...\n"
               ...
               "  -r <address> <prefix-length> for the forwarding route,转发路径\n"
               "  -d <address> for the domain name server域名服务器\n"
...
//    LOG("argv[0]:%s,argv[1]:%s,argv[2]:%s,argv[3]:%s,argv[4]:%s,argv[5]:%s",argv[0],argv[1],argv[2],argv[2],argv[3],argv[4],argv[5]);
